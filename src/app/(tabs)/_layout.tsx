...
export default function TabsLayout() {
  ...
  return (
    <Tabs screenOptions={({ route }) => ({
      ...
      tabBarIcon: ({ color, size }) => <Ionicons name={icons[route.name as keyof typeof icons] ?? 'ellipse'} size={route.name === 'add' ? size + 10 : size} color={route.name === 'add' ? colors.secondary : color} />,
    })}>
